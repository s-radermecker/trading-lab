...
from core.run_registry import load_run_registry, register_pipeline_run
...
def main() -> None:
    ...
    result = run_pipeline_v1(
        info_objects=infos,
        asset_scope=["EURUSD"],
        pipeline_id="registry_test_001",
    )

    run_file_path = save_pipeline_run(
        pipeline_result=result,
        output_dir="lab_runs",
    )

    registry_file_path = register_pipeline_run(
        pipeline_result=result,
        run_file_path=run_file_path,
        registry_path="lab_runs/run_registry.json",
    )

    registry_data = load_run_registry(
        registry_path="lab_runs/run_registry.json",
    )

    print(run_file_path)
    print(registry_file_path)
    print(registry_data[-1])
    print(f"registry_size={len(registry_data)}")
